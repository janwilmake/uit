...
13. Implement `uithub.llms` plugin
14. Many other plugins!!!
15. Add ability to configure a `dev` plugin with cookie for remote development with uithub as DX interface for testing.
...
Deploy to Cloudflare should become a plugin for cloudflare compatible projects. Same for vercel, same for github, gitlip, etc. The readme now has these buttons already, but it's a good value add once we do filters.

# MCP Button

Instead of the chat, release a AUTHENTICATABLE git mcp; `Chat with LLM` should use Git MCP
